...
export const nextStops = [
  {
    eyebrow: 'Start Here',
    title: 'Install Keel',
    body: 'Pick a local install path, verify the CLI, and prepare a repo for its first Keel scaffold.',
    href: '/docs/start-here/install-keel',
    cta: 'Go to installation',
  },
  {
    eyebrow: 'First Turn',
    title: 'Scaffold a project and make the first move',
    body:
...
That rule still holds when Keel is embedded inside a larger runtime. Keel
continues to own planning truth. A runtime such as Keeper can own provider
ingress, routing, execution, and replay, but it should lower external work
through a formal mission-request boundary instead of inventing board state
out-of-band.

If you are integrating Keel with GitHub issues or a multiplayer runtime, read
[Keeper Boundary And Mission Requests](/docs/foundations/keeper-boundaries-and-mission-requests).
