...
export const upstreamBoundary = {
  ...
  items: [
    'Core principles such as gardening first, heartbeat hygiene, and notification discipline.',
...
One example of upstream intent worth preserving exactly is heartbeat semantics: a missing heartbeat is acceptable, while a dirty heartbeat is warning-level loop debt that should be closed at the commit boundary. Downstream copies should preserve that contract instead of inventing repo-local heartbeat rules.
...
1. Start from upstream `AGENTS.md` and `INSTRUCTIONS.md`. Do not author these files from a blank page.
