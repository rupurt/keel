---
title: Keel Turn Loop
sidebar_label: The Turn Loop
...
export const turnLoopExamples = [
  ...
  {
    phase: 'Inspect',
    commands: ['keel mission next --status', 'keel pulse'],
  },
  {
    phase: 'Pull',
    commands: ['keel next --role manager'],
  },
  {
    phase: 'Ship',
    commands: [
  ...
  {
    phase: 'Close',
    commands: ['keel story accept --role manager STORY-ID'],
  },
];

After [Board Model](/docs/foundations/board-model) explains what exists and [Roles And Lanes](/docs/foundations/roles-and-lanes) explains who may act, the turn loop explains how the system changes over time. Keel is built around the idea that one turn should leave the board in a visibly better state. The loop below is not ceremony for its own sake. It is the minimum rhythm that keeps a human and AI team aligned on what is true.
...
If this repository participates in a Mission Stack, `keel turn` is also where
the local operator should see stack-specific turn state without losing the
single-repo loop:
...
`keel turn --json` should mirror the same facts in a dedicated stack payload so
automation and human output stay aligned.
...
Mission Stack does not redefine the pacemaker. `keel heartbeat` still answers
repo-local wake questions only. Stack readiness belongs on `turn`, `next`,
`mission next`, and `doctor`, not inside the heartbeat fields.
...
- [Board Model](/docs/foundations/board-model): what exists
- [Roles And Lanes](/docs/foundations/roles-and-lanes): who may act
- [The Turn Loop](/docs/workflows/turn-loop): how state changes
