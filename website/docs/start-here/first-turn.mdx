...
export const freshBoardSignals = [
  ...
  },
  {
    ...
    body:
      'Running `keel init` inside a git repository lets Keel install the commit hooks that keep heartbeat hygiene attached to the commit boundary.',
  },
  {
    eyebrow: 'Practical',
...
When `keel init` runs inside Git, it creates the board scaffold and installs Keel's git hooks for heartbeat closure at commit time.
...
A brand-new board usually does not have `.keel/heartbeat` yet. That is fine. The heartbeat is a wake marker, not a required structural file. It matters once you start poking the board or crossing the commit boundary.
