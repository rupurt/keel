...
export const freshBoardSignals = [
  ...
  },
  {
    ...
    body:
      'Running `keel init` inside a git repository lets Keel install the commit hooks that keep validation attached to the commit boundary.',
  },
  {
    eyebrow: 'Practical',
...
When `keel init` runs inside Git, it creates the board scaffold and installs Keel's git hooks for commit-boundary validation.
...
- `keel heartbeat` shows whether recent repository activity is still energizing the board
...
`keel heartbeat` is the canonical pacemaker surface now. If you want the deeper contract behind that command and the scenes that depend on it, read [Heartbeat And Pacemaker](/docs/cli/heartbeat-and-pacemaker) and [Scene Surfaces](/docs/cli/scene-surfaces).
