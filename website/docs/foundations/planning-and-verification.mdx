---
title: Keel Planning And Verification
sidebar_label: Planning And Verification
...
- in a Mission Stack repo, `keel doctor` should also report stack violations
  such as wrong branch, unauthorized active member, missing checkpoint
  acknowledgment, or unsupported foreign execution state
- `keel doctor --json` should expose those same violations as structured
  diagnostics instead of prose only
