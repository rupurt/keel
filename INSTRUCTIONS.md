...
5.  **Log & Commit**:
    ...
    - **Automated Guardrails**: The git pre-commit hook (installed via `just keel hooks install`) automatically executes `just quality` and `just test` before allowing the commit to proceed. 
