...
|----|-------------|--------------|
| MG-01 | Replace the synthetic `.keel/heartbeat` file as Keel's primary activity signal with a derived heartbeat model exposed through `keel heartbeat`, and have `keel flow --scene` use that derived activity to decide when the system is unplugged. | board: ... |
